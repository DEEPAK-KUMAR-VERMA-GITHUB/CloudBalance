...
export default function GenericTable<T>({
  ...
  const Row = ({ index, style }: { index: number; style: any }) => {
    if (index === flatRows.length) {
      loadMore();
      return (
        <div style={style} className="flex items-center justify-center text-gray-500" role="row">
    ...
    if (item.type === "group") {
      return (
        <div
          style={style}
          className="bg-gray-100 font-semibold px-4 py-2 border-b"
          role="row"
    ...
    return (
      <div
        ...
        style={{
          ...style,
          display: "grid",
          gridTemplateColumns: `40px ${visibleColumns
            .map((c) => c.width)
            .join("px ")}px 40px`,
        }}
